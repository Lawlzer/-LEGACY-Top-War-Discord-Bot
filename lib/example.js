...
exports.setCommand = async function (bot, message, argsLowercase, content) {
    ...
    var argsOriginalCase = content.split(' ').slice(1); // we want to save the command with caps, so we need to make normal args
    ...
    const newCommand = {
        ...
        commandDescription: commandDescriptionOriginalCase,
    }
...
exports.findUserCommand = async function(bot, message, database, inputCommand) {
    ...
    for (let i = 0; i < allCommands.length; i++) {
      if (inputCommand == allCommands[i].commandTrigger.toLowerCase()) {
        ...
        message.channel.send(allCommands[i].commandDescription); 
        console.log(allCommands[i].commandDescription); 
        return true; 
      }
    }
